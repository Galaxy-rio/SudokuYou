...
class HumanSolver(
    ...
    companion object {
        ...
        fun defaultDetectors(): List<TechniqueDetector> = listOf(
            ...
            FinnedFishDetector(size = 3),
            FinnedFishDetector(size = 4),
            BugPlusOneDetector(),
            XChainDetector(),
            WXYZWingDetector(),
            SueDeCoqDetector(subsetSize = 2),
            SueDeCoqDetector(subsetSize = 3),
            NYWingDetector(size = 5),
            NYWingDetector(size = 6),
            NYWingDetector(size = 7),
            NYWingDetector(size = 8),
            NYWingDetector(size = 9),
            BasicFishDetector(size = 5),
            BasicFishDetector(size = 6),
            BasicFishDetector(size = 7),
            GroupedXChainDetector(),
            XYChainDetector(),
            ThreeDMedusaDetector(),
            AicDetector(),
